...
async function handleActivityAction(button, action, resultAction) {
    ...
    // Bloquear ambos botones y mostrar spinner en el pulsado
    const siblingBtn = activityCard.querySelector(isApprove ? ".btn-reject" : ".btn-approve");
    ...
    if (siblingBtn) {
        siblingBtn.dataset.loading = "true";
        siblingBtn.style.pointerEvents = "none";
    }
    ...
    try {
        ...
        if (result.success) {
            ...
            if (siblingBtn) {
                delete siblingBtn.dataset.loading;
                siblingBtn.style.pointerEvents = "";
            }
            showAlert("Error", result.message, "error");
        }
    } catch (error) {
        // Restaurar botones si hay excepción
        delete button.dataset.loading;
        button.innerHTML = originalHTML;
        button.style.pointerEvents = "";
        if (siblingBtn) {
 ...
 * @returns {Promise<void>}
 */
async function updateAfterAction(type, id, action) {
    if (type === "activity") {
    ...
    // Recargar stats desde el servidor en lugar de calcularlas localmente
    try {
        const response = await fetch("index.php?accion=getPendingActivities");
        const result = await response.json();
        if (result.success) {
            controlPanelData.stats = result.stats;
        }
    } catch {
        // Si falla, dejamos las stats como están
    }
