...
if (isset($_POST['action']) && $_POST['action'] === 'run_phpunit') {
    ...
    if (!array_key_exists($suite, $suites)) { echo json_encode(['error' => 'Suite no valida']); exit; }
    ...
    if (!is_resource($proc)) { echo json_encode(['error' => 'No se pudo lanzar PHPUnit']); exit; }
...
<head>
  ...
  <style>
    ...
    .suite-btn:hover { background: rgba(0,0,0,.2); color: #fff; }
    .suite-btn.active {
    ...
    .suite-btn i { width: 16px; text-align: center; font-size: .85rem; }
    .suite-count {
      margin-left: auto;
    ...
    .btn-run   { background: var(--brand-primary); color: #fff; }
    .btn-bench { background: var(--accent-purple, #9b59b6); color: #fff; }
    .btn-run:hover, .btn-bench:hover   { opacity: .85; }
    .btn-run:disabled, .btn-bench:disabled { opacity: .45; cursor: default; }
    ...
    .t-pass  { color: var(--c-green);   }
    .t-fail  { color: var(--c-red);     }
    .t-error { color: var(--c-yellow);  }
    .t-suite { color: var(--brand-primary); font-weight: 700; display: block; margin-top: .6rem; }
    .t-ok    { color: var(--c-green);   font-weight: 700; }
    .t-err-sum { color: var(--c-red);   font-weight: 700; }
    .t-time  { color: var(--text-secondary); font-size: .74rem; }

    /* Spinner */
    .fa-spinner { font-size: .85rem; }
    ...
    .form-field input:focus { border-color: var(--brand-primary); }
    ...
    @media (max-width: 700px) {
      .runner-side { width: 54px; }
      .suite-btn .lbl, .side-label, .suite-count { display: none; }
      .suite-btn { padding: .85rem; justify-content: center; }
      .runner-main { padding: 1rem; }
    }
  </style>
</head>
<body>
...
<div class="runner-wrap">
  ...
  <nav class="runner-side" aria-label="Suites de test">
    ...
    <button class="suite-btn active" onclick="selectSuite('all',this,'Todas las suites','Ejecuta mocks + integracion + seguridad en una sola pasada.')">
      <i class="fas fa-play-circle"></i><span class="lbl">Todas</span><span class="suite-count">34</span>
    </button>
    <button class="suite-btn" onclick="selectSuite('mock',this,'Mocks — UserMockTest','Pruebas unitarias puras con PDO simulado. No requieren MySQL.')">
      <i class="fas fa-clone"></i><span class="lbl">Mocks</span><span class="suite-count">9</span>
    </button>
    <button class="suite-btn" onclick="selectSuite('login',this,'Login','Pruebas de inicio de sesion con base de datos real.')">
      <i class="fas fa-lock"></i><span class="lbl">Login</span><span class="suite-count">3</span>
    </button>
    <button class="suite-btn" onclick="selectSuite('register',this,'Registro','Pruebas de registro de nuevos usuarios.')">
      <i class="fas fa-user-plus"></i><span class="lbl">Registro</span><span class="suite-count">3</span>
    </button>
    <button class="suite-btn" onclick="selectSuite('edit',this,'Edicion de perfil','Actualizacion de datos y contrasena de usuario.')">
      <i class="fas fa-user-edit"></i><span class="lbl">Edicion</span><span class="suite-count">2</span>
    </button>
    <button class="suite-btn" onclick="selectSuite('activity',this,'Actividades y Peticiones','Creacion de actividades y peticiones con BD real.')">
      <i class="fas fa-running"></i><span class="lbl">Actividades</span><span class="suite-count">2</span>
    </button>
    <button class="suite-btn" onclick="selectSuite('security',this,'Seguridad','Inyeccion SQL, hashing bcrypt y entradas extremas.')">
  ...
  <div id="view-benchmark" class="runner-main view">
    ...
    <div class="output-panel">
      ...
      <div class="bench-actions">
        ...
        <a id="report-link" class="btn-secondary" href="performance/report.html"
